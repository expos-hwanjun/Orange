...
uses
  ...
  AdvSmoothButton, Vcl.WinXCtrls, Vcl.WinXPanels;
...
type
  TCard_F = class(TForm)
    ...
    procedure ApprovalAmtEditEnter(Sender: TObject);
    procedure ApprovalAmtEditPropertiesChange(Sender: TObject);
...
begin
  ...
  CatApprovalButton.Visible := (GetOption(379) = vanVCat) and (GetOption(495) = '1') and not Common.Config.PosCatUse;
  ...
  if (GetOption(379) = vanCat) and (Common.Config.van_trd in [vanFDIK,vanSmartro,vanKOCES,vanSPC,vanKIS]) then
    UnionPayCheckBox.Visible := True;
  if (GetOption(379) = vanVCat) and (Common.Config.van_trd in [vanKSNET,vanJTNET,vanKOVAN,vanNICE]) then
    UnionPayCheckBox.Visible := True;


  EasyPayButton.Visible := Common.Config.VanEasyPay <> 'N';
  //금결원은 멀티패드사용하면 간편결제 통합
  if (Common.Config.van_trd in [vanKFTC, vanKOVAN]) and (GetOption(379)=vanVCat) then
    EasyPayButton.Caption := '제로페이';

  if (Common.Config.van_trd in [vanKoces, vanJtnet, vanKis, vanSmartro, vanDaou, vanSPC, vanFDIK, vanKOVAN, vanKCP]) or ((Common.Config.van_trd = vanKSNET) and (Common.Config.VanEasyPay = 'T')) then
    EasyPayButton.Visible := false;

  if (Common.Config.van_trd in [vanKis, vanKoces]) and (Common.Config.VanEasyPay = 'S') then
    EasyPayButton.Visible := true;
...
var vTemp  :String;
    vIndex :Integer;
begin
  if not CatApprovalButton.Visible and not EasyPayButton.Visible  then
    ApprovalButton.Left := Self.Width div 2 - ApprovalButton.Width div 2
  else
  if not CatApprovalButton.Visible and EasyPayButton.Visible  then
  begin
    EasyPayButton.Left  := CatApprovalButton.Left;
    ApprovalButton.Left := Self.Width div 2 - ApprovalButton.Width div 2;
  end;


  if not Common.IsDebugMode and ((GetOption(379) = vanCat) or (Common.Config.van_trd <> vanKICC)) then
    BlockInput(true);
  try
...
begin
  ...
  begin
    ...
    if not Approval and (Common.Card.Yn_Cat = 'Y') and (GetOption(379) =  vanVCat)  then
    ...
    if GetOption(379) = vanCat then
      AppType := atCatCard
    else if (GetOption(379) = vanVCat) or (VAN in [vtKOCES, vtSmartro, vtNICE]) then
      AppType := atVCatCard;
    ...
    begin
      ...
      if ((VAN = vtKoces) and (PayCode = 'TE')) or ((VAN = vtFDIK) and (Common.Card.Ds_Card = 'K')) then
      begin
...
//        Common.PreSent.KaKaoDc    := Common.PreSent.KaKaoDc + Common.Card.DcAmt;
        ...
        if (GetOption(379) = vanCat) and (vRequestSaleAmt <> Common.ICCard.AgreeAmt) then
        begin
          Common.ErrBox(Format('포스에서 승인요청금액(%d)과'#13'단말기에서 실제 승인금액(%d)이 다릅니다'#13#13'반드시 관리대리점 확인바랍니다',[vRequestSaleAmt, Common.ICCard.AgreeAmt]));
        end;
        if VAN = vtKOVAN then
          Sleep(1000);
        ...
        //카드제거 음성
        if (GetOption(55) = '1') and (GetOption(379) = vanCat) then
        begin
  ...
  try
    if ((Common.Card.Ds_trd = dtApproval) and
      ( ((GetOption(63) = '1') and (GetAmtEdit.Value < 50000))
       ...
       or (Common.KFTCDetectData <> '')) ) or (Common.Card.Ds_trd = dtCancel)
       then
    begin
...
procedure TCard_F.ApprovalAmtEditEnter(Sender: TObject);
begin
  ApprovalAmtEdit.SelectAll;
end;
...
procedure TCard_F.ApprovalAmtEditPropertiesChange(Sender: TObject);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      MessageLabel.Caption := Common.GetPaPago('승인요청 중 입니다...잠시만 기다려주세요');
      try
...
procedure TCard_F.cxLabel2Click(Sender: TObject);
begin
  ApprovalAmtEdit.SelectAll;
  ApprovalAmtEdit.SetFocus;
end;
