...
begin
  if not Common.IsDebugMode and ((GetOption(379) <> '2') or (Common.Config.van_trd <> vanKICC)) then
    BlockInput(true);
  try
    Common.Device.SetJtentDetect(true);
...
begin
  ...
  begin
    ...
    case Common.Config.van_trd of
      ...
      1 : begin
            VAN    := vtDaou;
            if Common.Config.EtcSign then Common.HideWaitForm;
          end;
      2 : VAN    := vtNICE;
      3 :
          begin
      ...
      9 :
         begin
           ...
           if (Common.Config.VanEasyPay = 'M') and (GetOption(379)='0') then
             EasyPay := true;
    ...
    begin
      ...
      begin
        ...
        Common.Config.Van_Pass  := Common.Corner[vIndex].VanPWD;
        ...
        PassWord   := Common.Config.Van_Pass;
    ...
    EtcSign    := Common.Config.EtcSignPort > 0;
    EtcSignPort:= Common.Config.EtcSignPort;
    ...
    WorkingKey   := Common.Config.VanWorkingKey;
    ...
    try
      ...
      Common.Device.SetJtentDetect(false);
      ...
      begin
        ...
        vRequestSaleAmt := Common.ICCard.SaleAmt;
        if (Common.Card.Yn_NoCVM = 'N') and (VAN = vtKSNET) then
          EtcSign := true;
      end;
      isExecute := false;
...
//        BlockInput(false);
      ...
      begin
        ...
        //KIS or Star 밴으로 전자서명을 사용하는데 전자서명 데이터가 없을때는 영수증에 서명을 받으라구 한다
        if (CardType = 'C') and (VAN in [vtKIS, vtDaou, vtKOCES, vtKICC, vtKFTC, vtSmartro, vtFDIK, vtJTNET, vtNICE, vtKSNET, vtKCP, vtSPC]) and EtcSign and (ImgFileName = '') and not UnionPay and (AppType = atCard) then
        begin
          Common.HideWaitForm;
          if IsDesc = '' then
            Common.MsgBox('전자서명이 정상적으로 처리되지'+#13+'않았습니다'+#13#13
                         +'영수증에 서명을 받으십시오')
          else
            Common.MsgBox('DESC 가맹점이 아닙니다'+#13#13+
                                '영수증에 서명을 받으십시오');
        end;
