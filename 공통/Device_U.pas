...
PBaeminOrderMenu = ^TBaeminOrderMenu;
...
type
  ...
  private
    ...
    procedure AddRawPrintData(const aData:AnsiString);
  ...
  public
    ...
    function  SetImage(aFileName:String;aSize:Char;aCount:Integer=0):AnsiString;          //동적이미지데이터 생성
    ...
    function  GetSplitData(aArray:Array of String; aData:String):Boolean;
    procedure DeliverySave(aCompany, aDeliveryNo:String; aOrderAmt, aCardAmt :Integer; aBaeminOrderMenu:TList);
    function  CheckDeliveryMenuName(aValue:String):Boolean;
    ...
    procedure TableKeyRxChar(Com: TObject; Buffer: Pointer; Received: Cardinal);
    ...
    procedure OrderPrint(aCustomer:Boolean=True;aKitchen:Boolean=True);
    ...
    property OnTableKeyReadData : TGetStrProc read FOnTableKeyReadData write FOnTableKeyReadData;
    ...
    function BuildNaverQRData(aURL:AnsiString): AnsiString;
    function BytesToHex(const AData: TBytes): string;
    function BytesToStr(const AData: TBytes): String;

    procedure CmdToByte(BCmd: Byte);
    procedure CmdToOneByte(BCmd, BAdd, BMin: Byte);
    procedure CmdToTwoByte(BCmd, BAdd, BAdd1, BMin: Byte);
    procedure SendDispenserData(const AData: array of Byte; ALength: Integer);
...
//     KiccDSCX.OnRcvPinData := KiccDSCXRcvPinData;
   ...
   TableKeyComPort := TnrComm.Create(Application);
   TableKeyComPort.OnAfterReceive := TableKeyRxChar;
   ...
   FScannerTimer.Interval := 1;
   FScannerTimer.Enabled := False;
   FScannerTimer.OnTimer := ScannerTimerTimer;

   FTableKeyTimer := TTimer.Create(Application);
   FTableKeyTimer.Interval := 10;
   FTableKeyTimer.Enabled := False;
   FTableKeyTimer.OnTimer := TableKeyTimerTimer;
...
begin
  FreeAndNil(PadServer);
  FreeAndNil(FCidTimeOutTimer);
  FreeAndNil(FDispenserTimer);
  FreeAndNil(FCidTimer);
  FreeAndNil(FTableKeyTimer);
  FreeAndNil(FScannerTimer);

  FreeAndNil(BellComPort);
  FreeAndNil(LabelPrinterComPort);
  FreeAndNil(FixScannerComPort);
  FreeAndNil(TableKeyComPort);
  FreeAndNil(ScannerComPort);
  FreeAndNil(PrinterSocket);

  FreeAndNil(PrintComPort);
  FreeAndNil(LinkComPort);
  FreeAndNil(Link2ComPort);
  FreeAndNil(Link3ComPort);
  FreeAndNil(Link4ComPort);
  FreeAndNil(CidComPort);
  FreeAndNil(DispenserComPort);
  FreeAndNil(JtentICDetectComPort);
  FreeAndNil(SignImage);
  StopMonitor;
  inherited;
end;
////////////////////////////////////////////////////////////////////////////////
...
begin
  ...
  begin
    if FileExists(Format('%sDual\%s',[Common.AppPath,Common.Query.FieldByName('NM_CODE1').AsString])) then
      Common.DualData.Add( Common.Query.FieldByName('NM_CODE1').AsString );
    Common.Query.Next;
  end;
  Common.Query.Close;
  ...
  if TableKeyComPort.Active       then TableKeyComPort.Active := false;
  ...
  Common.ICCard.SmartPadIP := Common.GetIniFile('POS','스마트패드IP','');
  if Common.Config.SmartPad then
  begin
    PadServer.Active := false;
...
begin
  ...
  if (Common.Config.ReceiptPrinterDev = 0) or (Common.Config.ReceiptPrinterPort = '') or (Common.Config.ReceiptPrinterPort[1] in ['0','E']) then Exit;
  ...
  else if (GetOption(379) = vanCat) and (Common.PosType <> ptOnlyOrder) then
  begin
  ...
  PrintComPort.BufferInSize   := 4096 * 5;
  PrintComPort.BufferOutSize  := 4096 * 5;
  ...
  try
    ...
    if GetOption(472) = '1' then
    begin
      vGetTime := GetTickCount;
      while PrintComPort.Active and (vGetTime + 2000 > GetTickCount)do
      begin
        PrintComPort.Active := false;
...
begin
  if (GetOption(185) = '0') or ( Common.DeliveryLinkPrinter[0].InComPort = 0) then Exit;

  LinkPortData := EmptyStr;
  LinkComPort.ComPortNo := Common.DeliveryLinkPrinter[0].InComPort;
  LinkComPort.BufferInSize   := 4096 * 5;
  LinkComPort.BufferOutSize  := 4096 * 5;
...
begin
  if (GetOption(185) = '0') or (Common.DeliveryLinkPrinter[1].InComPort = 0) then Exit;
  Link2PortData := EmptyStr;
  Link2ComPort.ComPortNo      := Common.DeliveryLinkPrinter[1].InComPort;
  Link2ComPort.BufferInSize   := 4096 * 5;
  Link2ComPort.BufferOutSize  := 4096 * 5;
...
begin
  if (GetOption(185) = '0') or (Common.DeliveryLinkPrinter[2].InComPort = 0) then Exit;
  Link3PortData := EmptyStr;
  Link3ComPort.ComPortNo      := Common.DeliveryLinkPrinter[2].InComPort;
  Link3ComPort.BufferInSize   := 4096 * 5;
  Link3ComPort.BufferOutSize  := 4096 * 5;
...
begin
  if (GetOption(185) = '0') or (Common.DeliveryLinkPrinter[3].InComPort = 0) then Exit;
  Link4PortData := EmptyStr;
  Link4ComPort.ComPortNo      := Common.DeliveryLinkPrinter[3].InComPort;
  Link4ComPort.BufferInSize   := 4096 * 5;
  Link4ComPort.BufferOutSize  := 4096 * 5;
...
procedure TDevice.TableKeyPortOpen;
begin
  if Common.Config.TableKeyPort > 0 then
  begin
    TableKeyComPort.ComPortNo := Common.Config.TableKeyPort;
    TableKeyComPort.BufferInSize   := 4096;
    TableKeyComPort.Parity         := pNone;
    TableKeyComPort.StopBits       := sbOne;
    TableKeyComPort.StreamProtocol := spNone;
    TableKeyComPort.TraceStates    := [tsRxChar];
    TableKeyComPort.TimeoutRead    := 5000;
    TableKeyComPort.TimeoutWrite   := 5000;
    TableKeyComPort.BaudRate := 9600;
    try
      Sleep(100);
      TableKeyComPort.Active := true;
    except
      Common.ErrBox('테이블키를 초기화하지 못했습니다');
    end;
  end;
end;

procedure TDevice.LabelPrinterPortOpen;
begin
  ...
  LabelPrinterComPort.BufferInSize   := 4096 * 5;
  LabelPrinterComPort.BufferOutSize  := 4096 * 5;
...
procedure TDevice.BellPortOpen;
begin
...
begin
  ...
  if DispenserComPort.Active then
    DispenserComPort.Active := false;
...
function TDevice.PrinterCheck: String;
  ...
  begin
    try
      vGetTime := GetTickCount;
      while not PrintComPort.Active and (vGetTime + 2000 > GetTickCount) do
      begin
        PrintComPort.Active := true;
        Application.ProcessMessages;
      end;
...
var I, vCardAmt, vCashAmt, vCardCnt, vCatCnt, vTempCnt :Integer;
...
begin
  ...
  //재출력일때는 무조건 출력한다
  if (Common.RealPrintMode = pmRePrint) or Common.Config.IsKiosk then
  begin
  ...
  if (Common.CardPrintMode = cpmAtAcct) and (GetOption(407) = '1') and (vTempCnt > 0) then
  begin
    if Common.Config.SmartPad then
    begin
  ...
  else if (vCardCnt > 0) then
    vMustPrint := true
  else if (GetOption(8)='1') then
  begin
    if Common.AskBox('영수증을 출력하시겠습니까?') then
      vMustPrint := true
    else
  ...
  begin
     ...
     Common.PrintBuffer.Clear;
     //승인즉시 출력이 아니고 신용카드나 현금영수증일때 영수증을 출력하지 않을때
     if (Common.CardPrintMode <> cpmatOnce) and (GetOption(230)= '1') and ((vCardCnt > 0) or (Common.PreSent.CashRcpAmt <> 0)) then
       CardPrint(0)
     else
     ...
     begin
       ...
       case StoI(GetOption(125)) of
         ...
         1 : bPrint := True;                             //출력함
         2 : Common.RealPrintMode := pmNoPrint;
       end;
     end;
    ...
    {카드나 외상결제시 무조건 출력}
     ...
     begin
       ...
       PrintData := EmptyStr;
                                                      ...
                                                      //영수증 출력함        영수증 출력안함
  if (Common.PrintBuffer.Count > 0) or vMustPrint or (not vMustPrint and not vChoose and (GetOption(8)='1') and Common.AskBox('영수증을 출력하시겠습니까?')) then
  begin
    //영수증출력여부에 예를 했을때 0
    if vChoose and not vMustPrint then
    else
    begin
    ...
    if Common.PrintBuffer.Count > 0 then
      Common.BackupPrintBuffer.Clear;
  ...
  begin
    ...
    begin
       ...
       begin
          ...
          begin
            if Cells[GDM_DS_SALE, I] = 'D' then
              vPriceTxt := LPadB(FormatFloat('#,##0',StoI(Cells[GDM_PR_SALE_ORG, I]))+Common.Config.ServiceTxt, 27+pLen,' ')
            else
...
//            AddPrintData(' '+Replace(Cells[GDM_NM_ITEM, I],splitColumn, rptLF));
    ...
    AddPrintData(rptSizeWidth);
    AddPrintData('주문금액 '+LPad(FormatFloat('#,##0',Common.Present.WRcvAmt),12+(pLen div 2),' '));
  end;
  PrintPrinter(rptInterimReceipt);
end;
...
begin
   ...
   begin
     if (GetOption(27) = '0') or  (GetOption(27) = '2') then
     begin
       ...
       begin
         AddPrintData(rptSizeBoth);
         AddPrintData(rptAlignCenter);
         if (Common.Config.van_trd = vanKSNET) and (GetOption(379) = vanCat) then
           AddPrintData('    영  수  증')
         else
           AddPrintData('영  수  증');
         AddPrintData(rptAlignLeft);
     ...
     begin
       ...
       begin
          ...
          begin
             ...
             begin
               if Cells[GDM_DS_SALE, I] = 'D' then
                 vPriceTxt := LPadB(Common.Config.ServiceTxt, 9,' ')
               else
                 vPriceTxt := LPadB(FormatFloat('#,##0',   PrSale), 9,' ');
             end
             //메뉴를 두줄로 사용할때
     ...
     begin
        ...
        if KaKaoDc <> 0 then
          AddPrintData('                카카오할인'+LPadB(FormatFloat('#,##0',KaKaoDc*-1),16+pLen,' '));
        AddPrintData(rptCharNormal);
       AddPrintData(rptLF);
     end;
...
//     end;
     ...
     begin
       ...
       //외상잔액출력
       if (Common.Member.Code <> '') and (GetOption(373) = '1') and (Common.PreSent.TrustAmt <> 0) then
       begin
     ...
     if IsDebuggerPresent or (GetOption(492) = '1') then
       AddRawPrintData(BuildNaverQRData('https://nid.naver.com/nidlogin.login?realname=Y&svctype=262144&url=https://m.place.naver.com/my/checkin'));
...
//       DetailPrint;
     ...
     else if (GetOption(230) = '1') and (vCardCnt > 0) and (RealPrintMode <> pmRePrint) then
       PrintData := EmptyStr
     else
     begin
       if PrintData <> EmptyStr then
...
//        AddPrintData(rptLF);
      ...
      begin
        ...
        case StoI(GetOption(314)) of
          1 : AddPrintData(rptSizeNormal   + Format('호출번호 - %s%d',[Common.Config.DIDPickupPos, Common.PreSent.CallNo]));
          2 : AddPrintData(rptSizeHeight   + Format('호출번호 - %s%d',[Common.Config.DIDPickupPos, Common.PreSent.CallNo]));
          3 : AddPrintData(rptSizeBoth     + Format('호출번호 - %s%d',[Common.Config.DIDPickupPos, Common.PreSent.CallNo]));
          4 : AddPrintData(rptSize3Times   + Format('호출번호 - %s%d',[Common.Config.DIDPickupPos, Common.PreSent.CallNo]));
        end;
     ...
     if (GetOption(27) = '2') or (GetOption(27) = '3') then
     begin
       AddPrintData(rptAlignCenter + rptQRImage);
     end;
        ...
        begin
           ...
           case aKind of
             ...
             begin
               ...
               if (Common.SplitPrintMode = spmSplit) or (aKind = 3) or (GetOption(80) = '0') or (vTemp ='') then
               begin
                 if GetOption(382) = '1' then
                   AddPrintData('(고객용)');
                 AddPrintData(rptLF);
                 ...
                 else if (Common.SplitPrintMode = spmSplit) or (GetOption(80) = '0') or (vTemp ='') then
                   Cashier_Header(0, '');
               end;
             end;
             1 :
           ...
           begin
              ...
              begin
                ...
                AddPrintData('승인금액  : '+LPadB(FormatFloat('#,##0',StoI(Cells[GDC_AMT,I])-StoI(Cells[GDC_DCAMT,I])-StoI(Cells[GDC_AMT_CANCEL,I]))+' 원', 14, ' ') + vTemp);
                if StoI(Cells[GDC_DCAMT,I]) > 0 then
                  AddPrintData('할인금액  : '+LPadB(FormatFloat('#,##0',StoI(Cells[GDC_DCAMT,I]))+' 원', 14, ' '));
          ...
          {카드사제출용, 매장보관용에만 출력}
          //제이티넷 취소일때는 출력하지 않는다
          if ((Common.Config.van_trd = vanJTNET) and (GetOption(379) = vanVCat) and (Trim(Cells[GDC_DS_TRD,I]) = dtCancel)) then

          else
          begin
            if (GetOption(69) = '0') and (Trim(Cells[GDC_IMGFILE,I]) = '') and (GetOption(379) = vanVCat) and (StoI(Cells[GDC_AMT,I]) > 50000) and (Trim(Cells[GDC_DS_CARD,I]) <> atPG) then
            case aKind of
              1,2 :
              begin
                AddPrintData('--- 서 명 -------------------------------');
            ...
            else if (GetOption(69) = '0') and (GetOption(379) = vanVCat) then
            begin
              case I of
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (GetOption(69) = '0') and (Trim(Cells[GDC_IMGFILE,I]) = '') and (Cells[GDC_DS_CARD,I] <> atPG ) and (GetOption(379) = vanVCat) and (StoI(Cells[GDC_AMT,I]) > 50000) and (Trim(Cells[GDC_DS_CARD,I]) <> 'P') then
            case aKind of
              1,2 :
              begin
                AddPrintData('--- 서 명 -------------------------------');
            ...
            else if (GetOption(69) = '0') and (GetOption(379) = vanVCat) and (StoI(Cells[GDC_AMT,I]) > 50000)  then
            begin
              case I of
                0 : AddPrintData(rptSignImage0);
                1 : AddPrintData(rptSignImage1);
...
begin
   ...
   begin
    ...
    begin
      ...
      AddPrintData('승인금액  : '+LPadB(FormatFloat('#,##0',Amt-DcAmt-CancelAmt)+' 원', 14, ' ')+vTemp);
      if DcAmt > 0 then
        AddPrintData('할인금액  : '+LPadB(FormatFloat('#,##0',DcAmt)+' 원', 14, ' '));
    ...
    if (GetOption(69) = '0') and (Trim(ImgFile) = '') and (GetOption(379) = vanVCat) and (Amt > 50000) and (Ds_Card <> 'P') then
    case Kind of
      1,2 :
      begin
        AddPrintData(rptLF);
    ...
    else if (GetOption(69) = '0') and (GetOption(379) = vanVCat) and (Amt > 50000)  then
    begin
...
begin
  try
    ...
    begin
       ...
       begin
         OpenQuery('select b.CD_FLOOR, '
                  ...
                  +'       Ifnull(SUM(a.AMT_ORDER),0) as AMT_ORDER '
    ...
    on E: Exception do
    begin
      Common.WriteLog('CashierClosePrint',E.Message);
      Common.ErrBox(E.Message+#13#13+'계산원정산서 출력오류');
    end;
  end;
end;
...
begin
  try
    ...
    begin
      ...
      if Kind = 0 then
      begin
        OpenQuery('select Ifnull(Sum(SALE_CNT),0), '
                 +'       Ifnull(Sum(BAN_CNT),0), '
                 +'       Ifnull(Sum(VOID_CNT),0), '
                 +'       Ifnull(Sum(TOT_CNT),0) '
                 +'  from (  '
      ...
      begin
        ...
        begin
          ...
          AddPrintData(pStr+'   마감일시 : '+ FormatDateTime('yyyy년mm월dd일 hh시nn분', Now() ));
        Query.Close;
        ...
        OpenQuery('select min(COME_TIME) COME_TIME '
                 ...
                 [StoreCode,
                  WorkDate,
                  PosNo]);
        if not Query.Eof and (Query.FieldByName('COME_TIME').Value <> null) then
    ...
    on E: Exception do
    begin
      Common.WriteLog('PosClosePrint',E.Message);
      Common.ErrBox(E.Message+#13#13+'포스정산서 출력오류');
    end;
  end;
end;
...
begin
  ...
  begin
     ...
     begin
        ...
        While not Common.Query.Eof do
        begin
          if Common.Query.Fields[1].AsInteger <> 0 then
          begin
...
begin
  ...
  begin
     ...
     begin
        ...
        While not Common.Query.Eof do
        begin
           lsStr := RPadB(SCopy(Common.Query.Fields[0].AsString,1,18+pLen),18+pLen,' ')+
                    ...
                    Common.Query.Fields[4].AsString;
           AddPrintData(lsStr);
           Common.Query.Next;
        end;
        AddPrintData(rptTwoLine);
...
begin
  ...
  begin
    vTemp := Common.ShowNumberForm('패스워드를 입력하세요', 16,0,'');
...
procedure TDevice.AddRawPrintData(const aData:AnsiString);
begin
  PrintData := PrintData + aData;
end;
...
var I, vIdx, vMenuLen, vQtyLen, vCol, vIndex, vPrintQty :Integer;
    vCode,
    vTemp,
    vTemp1,
    vMenu,
...
begin
  ...
  begin
    ...
    if (GetOption(308)='1') and (aKitchenMenuName <> '') then
      aMenu := aKitchenMenuName;

    if aDsSale = 'D' then
      vMenu := aMenu+Common.Config.ServiceTxt
    else
      vMenu := aMenu;
    repeat
      if Pos(',',ACode) > 0 then
      ...
      begin
        ...
        case StoI(GetOption(103)) of
          ...
          begin
            ...
            vTemp := RPadB(vMenu,vMenuLen,' ');

            if aMaster <> EmptyStr then
              aMaster := RPadB(aMaster,  vMenuLen,' ');
            ...
            begin
              case StoI(GetOption(142)) of
                0 : vTemp := vTemp + LPadB(aAmt,  vQtyLen,' ')+#13;
                1 : vTemp := vTemp + LPadB(aQty,  vQtyLen,' ')+#13;
                2 : vTemp := vTemp + LPadB(aAmt+'('+aQty+')',  vQtyLen,' ')+#13;
              end;
              vTemp2    := vTemp;
              vPrintQty := 1;
            end
            ...
            begin
              //메뉴 수량별로 낱장을 출력하기 위함
              vTemp2  := vTemp + LPadB('1',  vQtyLen,' ')+#13;
              //부메뉴이면서 수량이 1이면 수량을 출력하지 않는다(햇갈리니까)
              if (aSubMenu <> '') and (aQty = '1') then
                vTemp   := vTemp + #13
              else
                vTemp   := vTemp + LPadB(aQty,  vQtyLen,' ')+#13;
              if aMaster <> EmptyStr then
              begin
                aMaster  := aMaster + LPadB(aMasterQty,  vQtyLen,' ')+#13;
                vMaster2 := aMaster + LPadB('1',         vQtyLen,' ')+#13;
              end;
              if AMenuType = 'W' then
                vPrintQty := 1
              else
          ...
          begin
            ...
            begin
              case StoI(GetOption(142)) of
                0 :  //금액출력
                begin
                  vMenuLen := 14+(vCol div 2);
                  vQtyLen  := 6;
                end;
                1 :  //수량출력
                begin
                  vMenuLen := 18+(vCol div 2);
                  vQtyLen  := 2;
                end;
                2 :  //금액+(수량) 출력
                begin
                  vMenuLen := 10+(vCol div 2);
                  vQtyLen  := 10;
                end;
              end;
            end
            ...
            begin
              ...
              //메뉴명이 길때 두번째줄 출력
              if (LengthB(vMenu) > vMenuLen) then
                vTemp := vTemp + SCopy(vMenu,vMenuLen+1,LengthB(vMenu)) +#13;
        ...
        begin
          vTemp  := aMaster  + vTemp;
          if aMemo <> '' then
            vTemp := vTemp + Ifthen(AMenuType[1] in ['C','S'],'  ','')+aMemo + #13;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  if (aSubMenu = '') or (Common.KitchenPrinter[vIdx].MenuList.Count=0) then
                    Common.KitchenPrinter[vIdx].MenuList.Add(vTemp2)
                  else
                    Common.KitchenPrinter[vIdx].MenuList.Strings[Common.KitchenPrinter[vIdx].MenuList.Count-vIndex] := Common.KitchenPrinter[vIdx].MenuList.Strings[Common.KitchenPrinter[vIdx].MenuList.Count-vIndex] +vTemp2;
...
//                                                      Common.KitchenPrinter[vIdx].BottomMargin );
  ...
  begin
    ...
    if (aDsSale = 'D') and (Pos(Common.Config.ServiceTxt,aMenu)=0) then
      aMenu := aMenu + Common.Config.ServiceTxt;

    //주방메모 출력
    if (GetOption(161) = '1') and (aMemo <> '') then
      aMenu := aMenu + Ifthen(AMenuType[1] in ['C','S'],'  ','') +aMemo;
    ...
    begin
      ...
      if GetOption(113) = '3' then
        vTemp := RPadB(aMenu,17+pLen,' ')
      else
        vTemp := RPadB(aMenu,38+pLen,' ');
    end;
...
var vIdx, I, aGroup, vMenuLen, vQtyLen, vCol   :Integer;
    aCode, vMenu, vAmt,
...
begin
          ...
          case StoI(GetOption(103)) of
            ...
            begin
              ...
              if LengthB(vMenu) <= vMenuLen then
                ...
                vTemp := RPadB(Scopy(vMenu,1,vMenuLen),vMenuLen,' ');
  ...
  begin
    ...
    begin
      if pLen = 0 then
        Common.CustomerCancel := Common.CustomerCancel + RPadB(vMenu, 17+pLen, ' ')
                                                       + LPadB('-'+vQty,   4, ' ')
      else
        Common.CustomerCancel := Common.CustomerCancel + RPadB(vMenu, 21+pLen, ' ')
                                                       + LPadB('-'+vQty,   4, ' ');
    end
    else
      //고객주문서 취소내역
...
function TDevice.SetDeliveryOrder(aCompany:String; var aDeliveryNo:String; var aOrderAmt:Integer; var aCardAmt:Integer; aValue: String; var aOrderMenu:TList):Boolean;
var vIndex, vMenuPosBegin, vMenuPosEnd, vPos :Integer;
    vTemp, vTemp1:String;
    vMenuList :^TBaeminOrderMenu;
    vMatchAmt :Integer;
    vOrderData: TArray<string>;
    vDcAmt,
    vDeliveryDcAmt,        //배달비할인
    vDeliveryAmt :Integer; //배달료
    vYogiYoType  :String;  //D:기본, M:중간크기
    vMenu :String;
begin
  Result := false;
  vTemp      := aValue;
  ...
  begin
    ...
    begin
      ...
      vTemp := Replace(vTemp, '2MB0'#1, '');
      vTemp := Replace(vTemp, '2MB', '');
    end
    else
    begin
      vTemp := Replace(vTemp, #$D#$A, splitColumn);
    ...
    if not GetSplitData(Common.DeliverySplit.YogiYoTitle, vTemp) then
    begin
      vTemp1 := '';
      for vIndex := 0 to High(Common.DeliverySplit.YogiYoTitle) do
        vTemp1 := vTemp1 + Format('%s,',[Common.DeliverySplit.YogiYoTitle[vIndex]]);

      Common.WriteLog('work', vTemp1+ '타이틀 불일치로 패스');
      Exit;
    end;
  end;
  ...
  begin
    ...
    if aCompany = 'C' then
    begin
      if GetSplitData(Common.DeliverySplit.CoupangDeliveryNo, vOrderData[vIndex]) or (Pos('[매장용]', vOrderData[vIndex]) > 0) then
        aDeliveryNo   :=Trim(vOrderData[vIndex+2]);

      //쿠파이츠일때
      if GetSplitData(Common.DeliverySplit.CoupangTotAmt, vOrderData[vIndex]) or (Pos('주문금액', vOrderData[vIndex]) > 0) then
      begin
      ...
      if (Pos('메뉴', vTemp) > 0) and (Pos('수량', vTemp) > 0) and (Pos('금액', vTemp) > 0) then
    ...
    begin
      ...
      if vYogiYoType = 'D' then
      begin
        if (GetSplitData(Common.DeliverySplit.YogiyoDeliveryNo, vOrderData[vIndex]) or (Pos('주문번호:', vOrderData[vIndex]) > 0)) and (Pos('#', vOrderData[vIndex]) = 0)  then
          aDeliveryNo   := CopyAnsi(vOrderData[vIndex],12,20);

        if GetSplitData(Common.DeliverySplit.YogiyoTotAmt, vOrderData[vIndex]) or (Pos('합계:', vOrderData[vIndex]) > 0) then
        begin
        ...
        //요기요 메뉴
        vTemp := Replace(vOrderData[vIndex],' ','');
        if (Pos('메뉴명', vTemp) > 0) and (Pos('수량', vTemp) > 0) and (Pos('가격', vTemp) > 0) then
          vMenuPosBegin := vIndex + 2;
      end
      else if vYogiYoType = 'M' then
      begin
        if (GetSplitData(Common.DeliverySplit.YogiyoDeliveryNo, vOrderData[vIndex]) or (Pos('주문 번호:', vOrderData[vIndex]) > 0)) and (Pos('#', vOrderData[vIndex]) = 0)  then
          aDeliveryNo   := GetOnlyNumber(CopyAnsi(vOrderData[vIndex],15,40));

        if GetSplitData(Common.DeliverySplit.YogiyoTotAmt, vOrderData[vIndex]) or (Pos('합 계 :', vOrderData[vIndex]) > 0) then
        begin
          aOrderAmt   := StrToInt(GetOnlyNumber(RightStr(vOrderData[vIndex],18)));
          aCardAmt    := aOrderAmt;
          vMenuPosEnd := vIndex -2;
  ...
  begin
    ...
    vMenuList^.TotalItem := '';
    vMenuList^.Step   := 0;
    vMenu             := Trim(vOrderData[vIndex]);
    vMenu             := Replace(vMenu, '└ ','');
    //주문메뉴가 한줄일때
    if aCompany = 'C' then   //쿠팡일때
    begin
      if (vMenu <> '') and (GetOnlyNumber(RightStr(Trim(vMenu),3)) = RightStr(Trim(vMenu),3)) then
      begin
        if Copy(vMenu,1,1) = '+' then
          vMenuList^.Name   := Trim(CopyAnsi(Trim(vMenu),1,26))
        else
          vMenuList^.Name   := Trim(CopyAnsi(Trim(vMenu),1,28));
        vMenuList^.Memo   := '';
        vMenuList^.Qty    := StrToIntDef(GetOnlyNumber(LeftStr(RightStr(Trim(vMenu),12),3)),1);
        vMenuList^.Amount := StrToIntDef(GetOnlyNumber(RightStr(Trim(vMenu),9)),0);
    ...
    begin
      ...
      begin
        ...
        begin
          if Copy(vMenu,1,1) = '-' then
            vMenuList^.Name   := Trim(CopyAnsi(Trim(vMenu),3,24))
          else
            vMenuList^.Name   := Trim(CopyAnsi(Trim(vMenu),1,26));
          vMenuList^.Memo   := '';
          vMenuList^.Qty    := StrToIntDef(GetOnlyNumber(LeftStr(RightStr(Trim(vMenu),15),4)),1);
          vMenuList^.Amount := StrToIntDef(GetOnlyNumber(RightStr(Trim(vMenu),11)),0);
          vMenuList^.Price  := vMenuList^.Amount div Ifthen(vMenuList^.Qty=0,1,vMenuList^.Qty);
        end;
      end
      else
      begin
        ...
        begin
          if Copy(vMenu,1,1) = '-' then
            vMenuList^.Name   := Trim(CopyAnsi(vMenu,3,24))
          else
            vMenuList^.Name   := Replace(Trim(CopyAnsi(vMenu,1,30)),' ','');
          vMenuList^.Memo   := '';
          vMenuList^.Qty    := StrToIntDef(GetOnlyNumber(LeftStr(RightStr(vMenu,32),6)),1);
          vMenuList^.Amount := StrToIntDef(GetOnlyNumber(RightStr(vMenu,15)),0);
          vMenuList^.Price  := vMenuList^.Amount div Ifthen(vMenuList^.Qty=0,1,vMenuList^.Qty);
        end;

      end;
    end;
    if Copy(vMenuList^.Name,1,2) = '+ ' then
      vMenuList^.Name := RightStr(vMenuList^.Name, Length(vMenuList^.Name)-2);

    aOrderMenu.Add(vMenuList);
  end;

  vMatchAmt := 0;
  for vIndex := 0 to aOrderMenu.Count-1 do
  begin
    if not CheckDeliveryMenuName(TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Name) then Continue;
    ...
    OpenQuery('select a.CD_MENU, '
             ...
             +'   and a.NM_BAEMIN =:P1 ',
             [Common.Config.StoreCode,
              TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Name]);
    if Common.Query.Eof then
    begin
    ...
    begin
      ...
      begin
        TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Code      := Common.Query.FieldByName('CD_MENU').AsString;
        TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).ClassCode := Common.Query.FieldByName('CD_CLASS').AsString;
        TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).DsTax     := Common.Query.FieldByName('DS_TAX').AsString;
        TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).MenuType  := Common.Query.FieldByName('DS_MENU_TYPE').AsString;
        vMatchAmt := vMatchAmt + TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Amount;
      end
      else
      begin
...
function TDevice.GetSplitData(aArray:Array of String; aData:String):Boolean;
var vIndex :Integer;
begin
  Result := false;
  for vIndex := 0 to High(aArray) do
  begin
    if Pos(aArray[vIndex], aData) > 0 then
    begin
      Result := true;
      Break;
    end;
...
procedure TDevice.DeliverySave(aCompany, aDeliveryNo:String; aOrderAmt, aCardAmt :Integer; aBaeminOrderMenu:TList);
var vRcpNo,
...
begin
  ...
  if aCompany = '쿠팡이츠' then
    vTemp := 'C'
  else if aCompany = '배달의민족' then
    vTemp := 'B'
  else if aCompany = '요기요' then
    vTemp := 'Y'
  else
    vTemp := '';
  ...
  vRcpNo := Common.Query.Fields[0].AsString;
  Common.Query.Close;

  Common.BeginTran;
  try
    if aCardAmt > 0 then
      ExecQuery('insert into SL_CARD (CD_STORE, '
               +'             				YMD_SALE, '
               +'             				NO_POS, '
               +'                     NO_RCP, '
               +'             				SEQ, '
               ...
               +'                    ''C'', '     //    TYPE_TRD, '
               +'                    '''', '      //    PG_TID, '
               +'             			 Date_Format(Now(), ''%Y%m%d%H%i%s''), '       //    NO_APPROVAL, '
               +'             			 '''', '      //    NO_CARD, '
               +'             			 :P4, '       //    AMT_APPROVAL, '
               +'             			 0, '         //    AMT_TIP, '
               +'             			 DivInt(:P4,11), '  //    AMT_VAT, '
               +'             			 ''00'', '      //    TERM_HALBU, '
               +'             			 '''', '      //    TRD_DATE, '
               +'             			 '''', '      //    TRD_TIME, '
               +'             			 '''', '      //    NM_CARDPL
    ...
    vSeq     := 1;
    vSaleAmt := 0;
    for vIndex := 0 to aBaeminOrderMenu.Count-1 do
    begin
      if TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Code = '' then
        Continue;

      if TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Item <> '' then
        Continue;
      ...
      ExecQuery('insert into SL_SALE_D(CD_STORE, '
               ...
               [Common.Config.StoreCode,
                ...
                Common.Config.PosNo,
                vRcpNo,
                TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Code,
                vSeq,
                TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Qty,
                TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Price,
                TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Amount,
                Ifthen(TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).DsTax = '1', Trunc(TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Amount / 11), 0),
                TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).DsTax,
                TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).ClassCode,
                TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).MenuType]);
      Inc(vSeq);
      vSaleAmt := vSaleAmt + TBaeminOrderMenu(aBaeminOrderMenu.Items[vIndex]^).Amount;
    end;

    //매칭되지 않은 금액이 있을때
    if vSaleAmt <> aOrderAmt then
      ExecQuery('insert into SL_SALE_D(CD_STORE, '
               +'					             YMD_SALE, '
               +'					             NO_POS, '
               +'					             NO_RCP, '
               ...
               +'              				 0, '
               +'              				 0, '
               +'              				 case when DS_TAX = ''1'' then DivInt(:P5, 11) else 0 end, '
               +'              				 DS_TAX, '
               +'              				 '''', '
               +'              				 '''', '
               +'              				 0, '
    ...
    ExecQuery('insert into SL_SALE_H(CD_STORE, '          //:P0, '
             +'                      YMD_SALE, '          //:P1, '
             +'                      NO_POS, '            //:P2, '
             +'                      NO_RCP, '            //:P3, '
             +'                      DS_SALE, '           //:P4, '
             ...
             +'                      AMT_TAX, '           //:P7, '
             +'                      AMT_DUTYFREE, '      //:P8, '
             +Ifthen(aCardAmt > 0, ' AMT_CARD, ','AMT_CASH, ')  //:P9
             +'                      AMT_LETSORDER, '     //:P10
             +'                      NO_TABLE, '          //:P11, '
             +'                      DT_SALE, '
             +'                      CD_SAWON, '
             ...
             [Common.Config.StoreCode,
              ...
              vRcpNo,
              aOrderAmt,
              aOrderAmt - Trunc(aOrderAmt / 11),       //AMT_DUTY
              Trunc(aOrderAmt / 11),                   //AMT_TAX
              0,
              aOrderAmt,
              0,
              0,
    ...
    begin
      ...
      Common.Table.Name       := Format('%s-%s',[Ifthen(aCompany='배달의미족','배민',aCompany),aDeliveryNo]);
      Common.Table.Memo       := '';
      ...
      begin
        ...
        if vKetchenCode <> '' then
          Common.Device.PrintKitchen(vKetchenCode,
                                     Common.Config.PackingTxt+Common.Query.FieldByName('NM_MENU_SHORT').AsString,
                                     '',
                                     '',
                                     ...
                                     0,
                                     Common.Query.FieldByName('QTY_SALE').AsInteger,
                                     'N',
                                     'Y',
    ...
    Common.WriteLog('work', Format('[전표번호] %s [%s주문번호] %s 저장완료',[vRcpNo, aCompany, aDeliveryNo]));
  except
    on E :Exception do
    begin
      Common.RollbackTran;
      Common.WriteLog('work', Format('%s주문 저장 오류 주문번호 %s 저장 중 %s',[aCompany, aDeliveryNo, E.Message]));
    end;
  end;
end;

function TDevice.CheckDeliveryMenuName(aValue:String):Boolean;
begin
  if (Pos('배달주소', aValue) > 0)
    or (Pos('접수번호', aValue) > 0)
    or (Pos('배달팁', aValue) > 0)
    or (Pos('배달 :', aValue) > 0)
    or (Pos('수저포크', aValue) > 0)
    or (Pos('주문번호', aValue) > 0)
    or (Pos('가게', aValue) > 0)
    or (Pos('가게', aValue) > 0)
    or (Pos('지번', aValue) > 0)
    or (Pos('도로명', aValue) > 0)
    ...
    or (Pos('010', aValue) > 0)
    or (Pos('연락처', aValue) > 0)
    or (Pos('요청사항', aValue) > 0)
    or (Pos('공동현관', aValue) > 0)
    or (Trim(aValue) = '') then
      Result := false
    else
      Result := true;
...
begin
  //IP가 같으면 윈도우 메세지로 보낸다,포스 1대 일때 주방과 영수증프린터 같이 사용합니다.
  if (Common.Config.ReceiptPrinterPort = aPort ) and ((GetOption(427) = '1') or (Common.Config.RcpToKitchen and (GetOption(379) = vanCat))) then
  begin
    PrintData := aData;
    PrintPrinter;
    Result := 'OK'
  ...
  else  if (Common.Config.PosIP = aHost) and ((aKind = '1')) then // and (GetOption(17) = '0')) then
  begin
    Common.WriteLog('work','주방출력');
...
begin
  ...
  //NICE Cat단말기를 연동시
  if (GetOption(379) = vanCat) and (Common.Config.van_trd = vanNICE) then Exit;
  ...
  if (GetOption(379) = vanCat) and (Common.Config.van_trd = vanFDIK) then
  begin
  ...
  begin
    try
      ...
      begin
        ...
        if (GetOption(379) = vanCat) and not (Common.Config.van_trd in [vanJTNET, vanKIS]) then Exit;

        if (Common.Config.van_trd in [vanKIS, vanJTNET]) then
        begin
          //이번거래만 단말기 연동으로 사용하기 위함
          Common.Config.Options[379]   := vanCat;
...
procedure TDevice.LinkRxChar(Com: TObject; Buffer: Pointer; Received: Cardinal);
  function SetDeliveryOrder(var aDeliveryNo:String; var aOrderAmt:Integer; aValue: String; var aOrderMenu:TList):Boolean;
  var vTemp :String;
  ...
  begin
    ...
    vMenuPosBegin := 0;
    vMenuPosEnd   := 0;
    if not GetSplitData(Common.DeliverySplit.BaeminTitle, vTemp) and (Pos('주문전표', vTemp) = 0)  then
      Exit;
    try
      ...
      for vIndex := 0 to Length(vOrderData)-1 do
      begin
        if GetSplitData(Common.DeliverySplit.BaeminDeliveryNo, vOrderData[vIndex]) then //or (Pos('주문번호:', vOrderData[vIndex]) > 0) then
          aDeliveryNo   := CopyAnsi(vOrderData[vIndex],10,12);

        //메뉴를 찾는다
        if (Pos('메뉴명', vOrderData[vIndex]) > 0) and (Pos('수량', vOrderData[vIndex]) > 0) and (Pos('금액', vOrderData[vIndex]) > 0) then
          vMenuPosBegin := vIndex + 2;

        if GetSplitData(Common.DeliverySplit.BaeminTotAmt, vOrderData[vIndex]) then // or (Pos('총 결제금액', vOrderData[vIndex]) > 0) or (Pos('만나서결제', vOrderData[vIndex]) > 0) then
        begin
          aOrderAmt   := StrToInt(GetOnlyNumber(RightStr(vOrderData[vIndex],7)));
          vMenuPosEnd := vIndex -2;
        end;
      ...
      begin
        ...
        begin
          ...
          vMenuList^.Step   := 0;
          //주문메뉴가 한줄일때
          if (vMenu <> '') and (GetOnlyNumber(RightStr(vMenu,3)) = RightStr(vMenu,3)) then
          begin
            vMenuList^.Name   := Trim(CopyAnsi(vMenu,1,27));
            vMenuList^.Qty    := StrToIntDef(GetOnlyNumber(CopyAnsi(Trim(vMenu),28,3)),1);
            vMenuList^.Amount := StrToIntDef(GetOnlyNumber(RightStr(Trim(vMenu),13)),0);
            vMenuList^.Price  := vMenuList^.Amount div Ifthen(vMenuList^.Qty=0,1,vMenuList^.Qty);
            vPos := vIndex;
          end
          else if vMenu <> '' then
          begin
            vMenuList^.Name   := vMenu;
            vMenuList^.Qty    := StrToIntDef(GetOnlyNumber(CopyAnsi(Trim(vOrderData[vIndex+1]),28,3)),1);
          ...
          for vIndex2 := 1 to 10 do
            if (vPos2+vIndex2 < vMenuPosEnd) and (CopyAnsi(Trim(vOrderData[vPos2+vIndex2]),2,1) = '+') and (Pos('0원)',Trim(vOrderData[vPos2+vIndex2])) > 0) then
            begin
              New(vMenuList);
              vMenuList^.Code        := '';
              vMenuList^.Item        := '';
              ...
              vMenuList^.Name   := Replace(vMenuList^.Name,'└ ','');
              vMenuList^.Qty    := 1;
              vMenuList^.Amount := 0;
              vMenuList^.Price  := 0;
              aOrderMenu.Add(vMenuList);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Code          := TBaeminOrderMenu(aOrderMenu.Items[vMainIndex]^).Code;
              TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Item          := Common.Query.FieldByName('CD_MENU').AsString;
              TBaeminOrderMenu(aOrderMenu.Items[vMainIndex]^).TotalItem := TBaeminOrderMenu(aOrderMenu.Items[vMainIndex]^).TotalItem + Format('%s|',[TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Item]);
              Inc(vStep);
              TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Step          := vStep;
            end
            else
            begin
              TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Code     := Common.Query.FieldByName('CD_MENU').AsString;
      ...
      on E :Exception do
      begin
        Common.WriteLog('work', Format('배민주문 파싱 오류 주문번호 %s 저장 중 %s',[aDeliveryNo, E.Message]));
      end;
    end;
  end;

var vReadData :AnsiString;
    vTableNo, vNotMathAmt, vIndex, I  :Integer;
    vDeliveryNo,
    vAddress,
    vTableMemo :String;
    vOrderMenu :TList;
...
begin
  ...
  begin
    Common.WriteLog('work', '[배민]'+LinkPortData);
    PrintData := LinkPortData;
    if (Common.Config.BaeminMenuCode <> '') and (Common.DeliveryLinkPrinter[0].Collection = 'Y') then
    begin
      try
        vOrderMenu := TList.Create;
        if SetDeliveryOrder(vDeliveryNo, vOrderAmt, PrintData, vOrderMenu) then
        begin
          if (vOrderAmt <> 0) and (GetOption(489)='0') then
            DeliverySave('배달의민족', vDeliveryNo, vOrderAmt, vOrderAmt, vOrderMenu)
          else if GetOption(489)='0' then
            Common.WriteLog('work', Format('배민(%s) 배달금액 %d원으로 패스',[vDeliveryNo, vOrderAmt]))
          else
            Common.WriteLog('work', Format('배민(%s) 수집만하는 옵션으로 패스',[vDeliveryNo]));
        end;
      finally
        for I := 0 to vOrderMenu.Count - 1 do
          Dispose(PBaeminOrderMenu(vOrderMenu[I]));
        vOrderMenu.Free;
      end;
    end
    else
      Common.WriteLog('work', '배달메뉴 미지정으로 패스');
...
begin
  ...
  begin
    Common.WriteLog('work','[쿠팡]'+Link2PortData);
    PrintData     := Link2PortData;
    if (Common.Config.BaeminMenuCode <> '') and (Common.DeliveryLinkPrinter[1].Collection = 'Y') then
    begin
      vOrderMenu := TList.Create;
      try
        if SetDeliveryOrder('C', vDeliveryNo, vOrderAmt, vCardAmt, PrintData, vOrderMenu) then
        begin
          if (vOrderAmt <> 0) and (GetOption(489)='0') then
            DeliverySave('쿠팡이츠', vDeliveryNo, vOrderAmt, vCardAmt, vOrderMenu)
          else if GetOption(489)='0' then
            Common.WriteLog('work', Format('쿠팡이츠(%s) 배달금액 %d원으로 패스',[vDeliveryNo, vOrderAmt]))
          else
            Common.WriteLog('work', Format('요기요(%s) 수집만하는 옵션으로 패스',[vDeliveryNo]));
        end;
      finally
        for I := 0 to vOrderMenu.Count - 1 do
          Dispose(PBaeminOrderMenu(vOrderMenu[I]));
...
begin
  ...
  begin
    Common.WriteLog('work','[요기요]'+Link3PortData);
    PrintData     := Link3PortData;
    if (Common.Config.BaeminMenuCode <> '') and (Common.DeliveryLinkPrinter[2].Collection = 'Y') then
    begin
      vOrderMenu := TList.Create;
      try
        if SetDeliveryOrder('Y', vDeliveryNo, vOrderAmt, vCardAmt, PrintData, vOrderMenu) then
        begin
          if (vOrderAmt <> 0) and (GetOption(489)='0') then
            DeliverySave('요기요', vDeliveryNo, vOrderAmt, vCardAmt, vOrderMenu)
          else if GetOption(489)='0' then
            Common.WriteLog('work', Format('요기요(%s) 배달금액 %d원으로 패스',[vDeliveryNo, vOrderAmt]))
          else
            Common.WriteLog('work', Format('요기요(%s) 수집만하는 옵션으로 패스',[vDeliveryNo]));
        end;
      finally
...
begin
  ...
  begin
    Common.WriteLog('work','[ETC]'+Link4PortData);
    PrintData     := Link4PortData;
    PrintData     := Replace(PrintData, #$D,'');
    PrintPrinter(rptLink4);
    PrintData     := EmptyStr;
    Link4PortData := EmptyStr;
  end;
end;
...
begin
  ...
  if Trim(vTempBuff) = '' then Exit;

  FCidBuff := FCidBuff + Trim(vTempBuff);
  Common.WriteLog('work', Format('CID : %s - %s',[vTempBuff,FCidBuff]));
...
procedure TDevice.TableKeyRxChar(Com: TObject; Buffer: Pointer; Received: Cardinal);
var
  vReadData : String;
  vIndex, i    : Integer;
begin
  ...
  FTableKeyBuff := FTableKeyBuff + vReadData;
  i := Pos(#13, FTableKeyBuff);
  if i = 0 then
    i := Pos(#3, FTableKeyBuff);
  if (Length(FTableKeyBuff) > 0) and ((FTableKeyBuff[1] = #2) or (FTableKeyBuff[1] = #10)) then
    System.Delete(FTableKeyBuff, 1, 1);

  if i > 0 then
  begin
    FTableKeyBuff := Replace(FTableKeyBuff, #2, '');
    FTableKeyBuff := Replace(FTableKeyBuff, #3, '');
    FTableKeyData := Copy(FTableKeyBuff, 1, i - 1);
    FTableKeyTimer.Enabled := True;
    FTableKeyBuff := '';
  end;
end;

// aType - 0:주문서 헤더가 필요없을때
procedure TDevice.PrintPrinter(aType:Integer);
  procedure InitPort;
  var
...
begin
  ...
  try
    ...
    case aType of
      rptNone,
      rptLink, rptLink2, rptLink3    : vPrintData := PrintData;
      rptLetsOrder, rptInterimReceipt :
      begin
        if Trim(PrintData) = '' then Exit;
      ...
      begin
        ...
        if  (vPrintData = EmptyStr) and (Trim(PrintData) = '') then Exit;
        if vPrintData = EmptyStr then
          vPrintData := PrinterCommendReplace(PrintData,
                                              Common.Config.ReceiptPrinterDev,
                                              Common.Config.PrintColum,
    ...
    begin
      ...
      begin
        //키오스크이면 한번은 다시 시작한다
        if Common.Config.IsKiosk and (vFailCount=0) then
        begin
          Inc(vFailCount);
          goto go_customer;
        end
        else if Common.AskBox('[ 고객주문서 ]'+#13+Common.KitchenPrinter[GetKitchenPrinterIndex(Common.Config.CustPrinterCode)].Name+' 프린터에'+#13+
                         ' 출력하지 못했습니다'+#13+
                         '다시시도 하시겠습니까?') then
        begin
          Inc(vFailCount);
    ...
    //배달의 민족 연결포트
    if (aType = rptLink) and (Common.DeliveryLinkPrinter[0].OutComPort <> '000') and not isCatKitchen then
    begin
      go_Link:
      Common.ShowWaitForm;
      if Common.isReceiptToKitehcn and Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].Link then
      begin
        goto go_receipt2;
        Common.HideWaitForm;
      end
        ...
        vRtn := OrderSend('1',
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].Code,
                           vPrintData,
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].IP,
                           ifThen(Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].Port = 'E',
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].EthernetIP,
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].Port)
                           );

      if (Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].Port = 'E') and (vRtn <> 'OK') and (vFailCount = 0) then
      begin
        Inc(vFailCount);
        goto go_Link;
      end;
      ...
      if vRtn <> 'OK' then
      begin
        if Common.AskBox('[ 배달전표 ]'+#13+Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[0].OutComPort)].Name+' 프린터에'+#13+
                         ' 출력하지 못했습니다'+#13+
                         '다시시도 하시겠습니까?') then
        begin
          Inc(vFailCount);
    ...
    //쿠팡이츠 연결포트
    if (aType = rptLink2) and (Common.DeliveryLinkPrinter[1].OutComPort <> '000') and not isCatKitchen then
    begin
      go_Link2:
      Common.ShowWaitForm;
      if Common.isReceiptToKitehcn and Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].Link then
      begin
        goto go_receipt2;
        Common.HideWaitForm;
      end
        ...
        vRtn := OrderSend('1',
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].Code,
                           vPrintData,
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].IP,
                           ifThen(Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].Port = 'E',
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].EthernetIP,
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].Port)
                           );

      if (Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].Port = 'E') and (vRtn <> 'OK') and (vFailCount = 0) then
      begin
        Inc(vFailCount);
        goto go_Link2;
      end;
      ...
      if vRtn <> 'OK' then
      begin
        if Common.AskBox('[ 쿠팡이츠전표 ]'+#13+Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[1].OutComPort)].Name+' 프린터에'+#13+
                         ' 출력하지 못했습니다'+#13+
                         '다시시도 하시겠습니까?') then
        begin
          Inc(vFailCount);
    ...
    //요기요 연결포트
    if (aType = rptLink3) and (Common.DeliveryLinkPrinter[2].OutComPort <> '000') and not isCatKitchen then
    begin
      go_Link3:
      Common.ShowWaitForm;
      if Common.isReceiptToKitehcn and Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].Link then
      begin
        goto go_receipt2;
        Common.HideWaitForm;
      end
        ...
        vRtn := OrderSend('1',
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].Code,
                           vPrintData,
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].IP,
                           ifThen(Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].Port = 'E',
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].EthernetIP,
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].Port)
                           );

      if (Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].Port = 'E') and (vRtn <> 'OK') and (vFailCount = 0) then
      begin
        Inc(vFailCount);
        goto go_Link3;
      end;
      ...
      if vRtn <> 'OK' then
      begin
        if Common.AskBox('[ 요기요전표 ]'+#13+Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[2].OutComPort)].Name+' 프린터에'+#13+
                         ' 출력하지 못했습니다'+#13+
                         '다시시도 하시겠습니까?') then
        begin
          Inc(vFailCount);
    ...
    //연결포트4
    if (aType = rptLink4) and (Common.DeliveryLinkPrinter[3].OutComPort <> '000') and not isCatKitchen then
    begin
      go_Link4:
      Common.ShowWaitForm;
      if Common.isReceiptToKitehcn and Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].Link then
      begin
        goto go_receipt2;
        Common.HideWaitForm;
      end
        ...
        vRtn := OrderSend('1',
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].Code,
                           vPrintData,
                           Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].IP,
                           ifThen(Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].Port = 'E',
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].EthernetIP,
                                  Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].Port)
                           );

      if (Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].Port = 'E') and (vRtn <> 'OK') and (vFailCount = 0) then
      begin
        Inc(vFailCount);
        goto go_Link4;
      end;
      ...
      if vRtn <> 'OK' then
      begin
        if Common.AskBox('[ 배달연결전표 ]'+#13+Common.KitchenPrinter[GetKitchenPrinterIndex(Common.DeliveryLinkPrinter[3].OutComPort)].Name+' 프린터에'+#13+
                         ' 출력하지 못했습니다'+#13+
                         '다시시도 하시겠습니까?') then
        begin
          Inc(vFailCount);
    ...
    begin
      ...
      Common.HideWaitForm;
      case aType of
        rptReceipt, rptReceipt1, rptInterimReceipt :
          if vRtn <> 'OK' then
          begin
        ...
        rptCustOrder :
          ...
          begin
            ...
            else if Common.AskBox('[ 고객주문서 ]'+#13+Common.KitchenPrinter[Common.Config.KitchenIndex].Name+' 프린터에'+#13+
                             ' 출력하지 못했습니다'+#13+
                             '다시시도 하시겠습니까?') then
            begin
              Inc(vFailCount);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            case aType of
              ...
              rptInterimReceipt : vTemp := '중간계산서를';
            end;
        ...
        try
          if (Common.Config.ReceiptPrinterDev <> prtKICC) and (GetOption(472)='1') then
          begin
            vGetTime := GetTickCount;
            while not PrintComPort.Active and (vGetTime + 2000 > GetTickCount)do
            begin
...
//              KiccDSCX.ReqSendRS232(vPrintData);
          ...
          //FDK 단말기 연동일때 일때
          else if (Common.Config.van_trd = vanFDIK) and (GetOption(379) = vanCat) then
          begin
          ...
          //단말기연동으로 사용시
          else if (GetOption(379) = vanCat) or (Common.Config.van_trd = vanKOVAN) then
          begin
            repeat
              ...
              if (Common.Config.van_trd in [vanKICC, vanKOVAN]) and (Length(vPrintData) < 50) then
                Sleep(10);
            until Length(vPrintData) = 0;
          end
          else
...
procedure TDevice.OrderPrint(aCustomer:Boolean=True;aKitchen:Boolean=True);
...
begin
  ...
  try
    //고객주문서를 출력한다고 체크했으면
    if (Common.OrderKind <> okChange) and aCustomer and
  ...
  try
    //주방주문서를 출력한다고 체크했으면
    if ((Common.OrderKind <> okChange) or (GetOption(408)='1')) and (aKitchen) then
    begin
      For I := 0 to High(Common.KitchenPrinter) do
      begin
...
begin
      ...
      begin
        ...
        begin
          case StoI(GetOption(96)) of
            ...
            5 : Result := Result + rptSize3Times + Common.KitchenPrinter[aIndex].Name+#13;
          end;
        end
        else
        begin
          case StoI(GetOption(96)) of
            ...
            5 : Result := Result + rptSize3Times + Common.Config.KitchenPrintTitle+#13;
          end;
        end;
        ...
        case StoI(GetOption(98)) of
          1 : Result := Result + rptSizeNormal + '주방명 :' + Common.KitchenPrinter[aIndex].Name+#13;
          2 : Result := Result + rptSizeHeight + '주방명:' + Common.KitchenPrinter[aIndex].Name+#13;
          3 : Result := Result + rptSizeBoth   + '주방명:' + Common.KitchenPrinter[aIndex].Name+#13;
        end;

        case StoI(GetOption(482)) of
          0 : Result := Result + rptAlignLeft;
          1 : Result := Result + rptAlignCenter;
          2 : Result := Result + rptAlignRight;
        end;
        ...
        case StoI(GetOption(101)) of
          ...
          begin
            case StoI(GetOption(103)) of
              ...
              2,3 : Result := Result + rptSizeBoth   + rptOneLine2+#13;
            end;
          end;
          1 :
          begin
        ...
        //실제주문메뉴내역
        case StoI(GetOption(103)) of
          0 : Result := Result + rptCharBold + rptSizeNormal + AValue;
          1 : Result := Result + rptCharBold + rptSizeHeight + AValue;
          2 : Result := Result + rptCharBold + rptSizeBoth + AValue;
          3 : Result := Result + rptCharBold + rptSize3Times + AValue;
        end;
        ...
        begin
          case StoI(GetOption(105)) of
            1: Result := Result + rptSizeNormal + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
            2: Result := Result + rptSizeHeight + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
            3: Result := Result + rptSizeBoth + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
          end;
        end
        else
        begin
          ...
          case StoI(GetOption(105)) of
            ...
            begin
              case StoI(GetOption(106)) of
                0: Result := Result + rptSizeNormal + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
                1: Result := Result + rptSizeNormal + RPadB(FormatDateTime('mm-dd hh:nn:ss', Now()),21,' ') + rptSizeNormal + FillChr(vCol)
                                    + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo),21,' ')+#13;
                2: Result := Result + rptSizeNormal + RPadB(FormatDateTime('mm-dd hh:nn:ss', Now()),21,' ') + rptSizeHeight + FillChr(vCol)
                                    + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo),21,' ')+#13;
                3: Result := Result + rptSizeNormal   + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeBoth
            ...
            begin
              case StoI(GetOption(106)) of
                0: Result := Result + rptSizeHeight + FormatDateTime('mm-dd hh:nn:ss', Now()) +#13;
                1: Result := Result + rptSizeHeight + RPadB(FormatDateTime('mm-dd hh:nn:ss', Now()),21,' ') + rptSizeNormal + FillChr(vCol)
                                    + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo),21,' ')+#13;
                2: Result := Result + rptSizeHeight + RPadB(FormatDateTime('mm-dd hh:nn:ss', Now()),21,' ') + rptSizeHeight + FillChr(vCol)
                                    + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo),21,' ')+#13;
                3: Result := Result + rptSizeHeight   + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeBoth
                                    + ' 주문번호 '+IntToStr(Common.Table.OrderNo)+#13;
              end;
            end;
            3 :
            begin
              case StoI(GetOption(106)) of
                0: Result := Result + rptSizeBoth + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
  ...
  case StoI(GetOption(99)) of
    ...
    begin
      ...
      Result := Result +   '이동시간: ' + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
    end;
    3 :
    begin
      Result := Result + rptSizeBoth;
...
function TDevice.PrinterCommendReplace(aValue:String;aDevice,aCol,aMagin:Integer):String;
  ...
  begin
    ...
    try
      TargetFile := TFileStream.Create(bFileName, fmCreate);
      try
...
begin
  ...
  begin
    Result := #27'@'+   // 프린터 초기화
              Ifthen((GetOption(27) = '1') or (GetOption(27) = '3'), #28#112#1#0, '') + // 그림인쇄
              AValue+
              vMargin+
              #27#74#0+ // 인쇄(Feed)
  ...
  begin
    Result := #27'@'+   // 프린터 초기화
            Ifthen((GetOption(27) = '1') or (GetOption(27) = '3'), #13#28#112#1#0, '') + // 그림인쇄
            AValue+
            vMargin+
            #27#74#0+ // 인쇄(Feed)
            #27'i';  // 컷팅
  ...
  if (Common.Config.van_trd in [vanKICC,vanNICE]) and (GetOption(379) = vanCat) then
     Result := Replace(Result, rptCharBold,      '');
  ...
  if (Common.Config.van_trd in [vanKCP,vanKSNET]) and (GetOption(379) = vanCat) then
    Result := Replace(Result, rptCharBold,      #27#33#8)
  else
    Result := Replace(Result, rptCharBold,      #27#71#1);

  Result := Replace(Result, rptQRImage,       #28#112#1#1);
...
begin
  ...
  with Common do
  begin
    if (KitchenPrinter[AIndex].Data = '') and (KitchenPrinter[AIndex].Cancel = '') then Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if CustomerCancel + CustomerPrinter <> '' then
        PrintData := PrintData + rptOneLine+#13;
    end;
    ...
    if not Common.OrderRePrint then
      ...
      begin
        case StoI(GetOption(117)) of
          ...
          begin
            case StoI(GetOption(116)) of
              0: PrintData := PrintData + rptSizeNormal + '주문시간   : '+ FormatDateTime('mm월 dd일 hh시 nn분', Now())+#13;
              1: PrintData := PrintData + rptSizeNormal + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeNormal
                                        + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo), 23+pLen, ' ')+#13;
              2: PrintData := PrintData + rptSizeNormal + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeHeight
                                        + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo), 23+pLen, ' ')+#13;
              3: PrintData := PrintData + rptSizeNormal + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeBoth
                                        + ' 주문번호 '+IntToStr(Common.Table.OrderNo)+#13;
            end;
          end;
          2 :
          begin
            case StoI(GetOption(116)) of
              0: PrintData := PrintData + rptSizeHeight + '주문시간   : '+ FormatDateTime('mm월 dd일 hh시 nn분', Now())+#13;
              1: PrintData := PrintData + rptSizeHeight + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeNormal
                                        + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo), 23+pLen, ' ')+#13;
              2: PrintData := PrintData + rptSizeHeight + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeHeight
                                        + LPadB('주문번호 : '+IntToStr(Common.Table.OrderNo), 23+pLen, ' ')+#13;
              3: PrintData := PrintData + rptSizeHeight + FormatDateTime('mm-dd hh:nn:ss', Now()) + rptSizeBoth
                                        + ' 주문번호 '+IntToStr(Common.Table.OrderNo)+#13;
            end;
          end;
          3 :
          begin
            case StoI(GetOption(116)) of
              0: PrintData := PrintData + rptSizeBoth + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
      ...
      begin
        case StoI(GetOption(117)) of
          1 : PrintData := PrintData + rptSizeNormal + '주문시간   : '+ FormatDateTime('mm월 dd일 hh시 nn분', Now())+#13;
          2 : PrintData := PrintData + rptSizeHeight + '주문시간   : '+ FormatDateTime('mm월 dd일 hh시 nn분', Now())+#13;
          3 : PrintData := PrintData + rptSizeBoth + FormatDateTime('mm-dd hh:nn:ss', Now())+#13;
        end;
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if Cells[GDM_DS_SALE, I] = 'D' then
              vPrice := LPadB(FormatFloat('#,##0',StoI(Cells[GDM_PR_SALE_ORG, I]))+Common.Config.ServiceTxt, 27+pLen,' ')
            else
...
function TDevice.SetImage(aFileName:String;aSize:Char;aCount:Integer): AnsiString;
var
  vRow : PByteArray;
...
procedure TDevice.TableKeyTimerTimer(Sender: TObject);
begin
  FTableKeyTimer.Enabled := False;
  if Assigned(FOnTableKeyReadData) then
    FOnTableKeyReadData(FTableKeyData);
end;
...
var vRow, vIndex, vCol  :Integer;
    lsStr :String;
    vTemp1, vTemp2, vTemp3 :Currency;
begin
  PrintData := EmptyStr;
  case aKind of
    0 :
    ...
    begin
      ...
      begin
        ...
        lsStr := lsStr   + LPadB(String(aGrid.DataController.Values[vRow, 2]),7,' ');
        lsStr := lsStr   + LPadB(FormatFloat('#,#0',aGrid.DataController.Values[vRow, 5]),12,' ');
        AddPrintData(lsStr);
      end;
      AddPrintData('    [ 합  계 ] '+ LPadB(FormatFloat(',0',aGrid.DataController.Summary.FooterSummaryValues[2]),27+pLen,' '));
      AddPrintData(rptOneLine);
    end;
    5 :
    begin
      ...
      AddPrintData(rptSizeNormal);
      AddPrintData('매 장 명 : '+Common.Config.StoreName);
      AddPrintData('조회년도 : '+aSaleDate);
      AddPrintData(rptTwoLine);
      AddPrintData(pStr+'월'+pStr+' 총매출금액     할인   서비스   매출금액');
      AddPrintData(rptTwoLine);
      For vRow := 0 to aGrid.DataController.RecordCount-1 do
      begin
        lsStr := Format('%s%11.11s%9.9s%9.9s%11.11s',[Copy(aGrid.DataController.Values[vRow, 0],6,2),
                                                              FormatFloat('#,#0',aGrid.DataController.Values[vRow, 1]),
                                                              FormatFloat('#,#0',aGrid.DataController.Values[vRow, 2]),
                                                              FormatFloat('#,#0',aGrid.DataController.Values[vRow, 3]),
                                                              FormatFloat('#,#0',aGrid.DataController.Values[vRow, 4])]);
        AddPrintData(lsStr);
      end;
    ...
    begin
      AddPrintData(rptSizeBoth);
      AddPrintData(rptAlignCenter);
      AddPrintData('전년대비 매출');
      AddPrintData(rptAlignLeft);
      AddPrintData(rptSizeNormal);
      AddPrintData('매 장 명 : '+Common.Config.StoreName);
      AddPrintData(rptTwoLine);
      AddPrintData(pStr+'  월 '+pStr+Format('       %s년      %s년       증감',[aGrid.DataController.Values[0, 0],aGrid.DataController.Values[0, 1]]));
      AddPrintData(rptTwoLine);
      vTemp1 := 0;
      vTemp2 := 0;
      vTemp3 := 0;
      For vCol := 1 to 12 do
      begin
        lsStr := Format(' %5.5s%13.13s%13.13s%12.12s',[aGrid.Columns[vCol].Caption,
                                                          FormatFloat('#,#0',aGrid.DataController.Values[0, vCol]),
                                                          FormatFloat('#,#0',aGrid.DataController.Values[1, vCol]),
                                                          FormatFloat('#,#0',aGrid.DataController.Values[1, vCol]-aGrid.DataController.Values[0, vCol])]);
        vTemp1 := vTemp1 + aGrid.DataController.Values[0, vCol];
        vTemp2 := vTemp2 + aGrid.DataController.Values[1, vCol];
        vTemp3 := vTemp1 + aGrid.DataController.Values[1, vCol]-aGrid.DataController.Values[0, vCol];

        AddPrintData(lsStr);
      end;
      AddPrintData(Format('[합계]%-13.13s%-13.13s%-11.11s',[FormatFloat('#,#0',vTemp1),FormatFloat('#,#0',vTemp2),FormatFloat('#,#0',vTemp3)]));
      AddPrintData(rptOneLine);
    end;
  end;
  AddPrintData('출력시간 : '+FormatDateTime('yyyy/mm/dd(ddd) ampm hh:nn ',now));
...
var
  vTotalBytes, vByteCount, I, vIndex: Integer;
  bTemp, fcc: Byte;
  strInData: string;
  vBufferBytes: TBytes;
  vStatus :Integer;
  vRecvData: array[0..1023] of Byte;
  vCashIn: array[0..5] of Byte;
  vCashOut: array[0..5] of Byte;

begin
  vTotalBytes := Received;
  if vTotalBytes <= 0 then
    Exit;

  for vIndex := 0 to Received - 1 do
    FDispenserBuff := FDispenserBuff + PAnsiChar(Buffer)[vIndex];

  SetLength(vBufferBytes, vTotalBytes);
  Move(Buffer^, vBufferBytes[0], vTotalBytes);
...
//    vCashOut[I] := 0;
//
//  for vByteCount := 0 to vTotalBytes - 1 do
//  begin
//    bTemp := vBufferBytes[vByteCount];
//
//    if (bTemp = $FE) and (Common.KioskCash.RecvPos < 3) then
//      Common.KioskCash.RecvPos := 0;
//
//    if bTemp = $FE then
//      strInData := strInData + sLineBreak;
//
//    strInData := strInData + IntToHex(bTemp, 2) + ' ';
//
//    if Common.KioskCash.RecvPos < Length(vRecvData) then
//    begin
//      vRecvData[Common.KioskCash.RecvPos] := bTemp;
//      Inc(Common.KioskCash.RecvPos);
//    end;
//
//    if Common.KioskCash.RecvPos >= 19 then
//    begin
//      fcc := 0;
//      for I := 0 to 18 do
//        fcc := fcc xor vRecvData[I];
//
//      if fcc = 0 then
//      begin
//        case vRecvData[2] of
//          $13:
//            begin
//              vStatus := vRecvData[4] shl 8 or vRecvData[3];
//              for I := 0 to 5 do
//                vCashIn[I] := vRecvData[5 + I];
//              for I := 0 to 4 do
//                vCashOut[I] := vRecvData[11 + I];
//              Common.KioskCash.Ver := IntToHex(vRecvData[17],2);
//            end;
//        end;
//      end;
//      Common.KioskCash.RecvPos := 0;
...
//  if strInData <> '' then
//  begin
//    Common.KioskCash.inAmount := vCashIn[0] + vCashIn[1] * 5 + vCashIn[2] * 10 + vCashIn[3] * 50 + vCashIn[4] * 100 + vCashIn[5] * 500;
//    Common.KioskCash.inAmount := Common.KioskCash.inAmount * 100;
//
//    Common.KioskCash.outAmount := vCashOut[0] + vCashOut[1] * 5 + vCashOut[2] * 10 + vCashOut[3] * 50;
//    Common.KioskCash.outAmount := Common.KioskCash.outAmount * 100;
//
//    Common.KioskCash.Pay := (vStatus and $08) = $08;
//    Common.KioskCash.HP1 := (vStatus and $40) = $40;
//    Common.KioskCash.HP2 := (vStatus and $80) = $80;
//    Common.KioskCash.BD1 := (vStatus and $100) = $100;
//    Common.KioskCash.BD2 := (vStatus and $200) = $200;
//  end;

  Common.WriteLog('work', Format('입출금기(OUT) - %s',[Common.KioskLogFormat(FDispenserBuff)]));
...
//       vPrintData := vPrintData + rptAlignCenter + rptQRImage + CopyPos(Common.PreSent.TicketPrintData.Strings[vIndex], #28, 0) + rptLF;
...
begin
  try
    ...
    else if (not Common.Config.RcpToKitchen and (Common.Config.ReceiptPrinterDev > 0)) or (Common.Config.RcpToKitchen and (GetOption(379) = vanCat))  then
    begin
      if aValue then
        PrintComPort.Active := true
      else
...
var
  ...
  vSl: TStringList;
  vI1, vI2: Integer;
  vCommand, vSuccess, vResult, vPadIP: string;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        //스마트패드 IP를 고정 또는 무선공유기 IP 대역이 다를때(대역이 다르면 공유기 GateWay IP가 옴)
        vPadIP := Common.GetIniFile('POS','스마트패드IP_FIX','');
        if vPadIP = '' then
        begin
          SmartPadHost := AContext.Connection.Socket.Binding.PeerIP;
          Common.ICCard.SmartPadIP := SmartPadHost;
        end
        else
        begin
          SmartPadHost := vPadIP;
        ...
        Common.SetIniFile('POS', '스마트패드IP', SmartPadHost);
      end
      // 요청결과 전문
...
begin
  ...
  vGetData   := EmptyStr;
  try
    with TIdUDPClient.Create(Application) do
    try
      Host := Ifthen(SmartPadHost='', Common.GetIniFile('POS','스마트패드IP',''), '');
      Port := 7007;
      try
        Connect;
      except
        Common.WriteLog('work', Format('스마트패드 전송실패(%s)',[Ifthen(SmartPadHost='', Common.GetIniFile('POS','스마트패드IP',''), '')]));
        Exit;
      end;
      ...
      if Pos('dual', aData) = 0 then
        Common.WriteLog('work', Format('스마트패드 전송-%s',[vSend]));
      Send(vSend,IndyTextEncoding_OSDefault);
      if Pos('dual', aData) = 0 then
        vGetData := ReceiveString(1000,IndyTextEncoding_OSDefault);
      Result := true;
    finally
      Free;
    end;
...
var
  ...
  vBytes :TBytes;
begin
  if not IsDebuggerPresent and (Common.Config.KioskDispenserPort = 0) then Exit;
  try
    FDispenserBuff    := '';
    FDispenserData    := '';
    ...
    if not DispenserComPort.Active then
      DispenserComPort.Active := true;
    Common.WriteLog('work', Format('입출금기(IN) - %s',[Common.KioskLogFormat(aData)]));

    vBytes := StrToBytes(aData);
    if Length(vBytes) > 0 then
      DispenserComPort.SendData(PAnsiChar(@vBytes[0]), Length(vBytes));
...
procedure TDevice.OnCardUID(const UID: string);
begin
  FScannerData := UID;
  FScannerTimer.Enabled := True;
  FScannerBuff := '';
...
function TDevice.Bytes(const Values: array of Byte): TBytes;
var
  I: Integer;
begin
...
function TDevice.AnsiToBytes(const S: AnsiString): TBytes;
var
  I: Integer;
begin
...
function TDevice.ConcatBytes(const Parts: array of TBytes): TBytes;
var
  Total, P, I: Integer;
begin
  Total := 0;
...
function TDevice.BuildNaverQRData(aURL:AnsiString): AnsiString;
var
  vResult :TBytes;
begin
  vResult := ConcatBytes([
...
//    Bytes([$1B, $40]),
    ...
    Bytes([
      $1D, $28, $6B,
      Byte((Length(aURL) + 3) and $FF),
      Byte(((Length(aURL) + 3) shr 8) and $FF),
      $31, $50, $30
    ]),
    AnsiToBytes(aURL),
...
//    Bytes([$1D, $56, $41, $00])
  ...
  if Length(vResult) > 0 then
    SetString(Result, PAnsiChar(@vResult[0]), Length(vResult))
  else
    Result := '';
end;

function TDevice.BytesToHex(const AData: TBytes): string;
var
  I: Integer;
begin
  Result := '';
...
function TDevice.BytesToStr(const AData: TBytes): string;
var I: Integer; SB: TStringBuilder;
begin
  SB := TStringBuilder.Create(Length(AData) * 3);
  try
    for I := 0 to High(AData) do
    begin
      if I > 0 then
        SB.Append(' ');
      SB.Append(IntToHex(AData[I], 2));
    end;
    Result := SB.ToString;
  finally
    SB.Free;
...
procedure TDevice.CmdToByte(BCmd: Byte);
var
  I: Integer;
begin
  InitKioskCashRecord(Common.KioskCash);
  Common.KioskCash.DataBytes[0] := $FE;

  if BCmd = $13 then
  begin
    Common.KioskCash.DataBytes[1] := 17;
    Common.KioskCash.DataBytes[2] := BCmd;
    Common.KioskCash.DataBytes[17] := 0;
    for I := 0 to 16 do
      Common.KioskCash.DataBytes[17] := Common.KioskCash.DataBytes[17] xor Common.KioskCash.DataBytes[I];
    SendDispenserData(Common.KioskCash.DataBytes, 18);
  end
  else
  begin
    Common.KioskCash.DataBytes[1] := 13;
    Common.KioskCash.DataBytes[2] := BCmd;
    Common.KioskCash.DataBytes[13] := 0;
    for I := 0 to 12 do
      Common.KioskCash.DataBytes[13] := Common.KioskCash.DataBytes[13] xor Common.KioskCash.DataBytes[I];
    SendDispenserData(Common.KioskCash.DataBytes, 14);
  end;
end;

procedure TDevice.CmdToOneByte(BCmd, BAdd, BMin: Byte);
var
  I: Integer;
begin
  InitKioskCashRecord(Common.KioskCash);
  Common.KioskCash.DataBytes[0] := $FE;
  Common.KioskCash.DataBytes[1] := 7;
  Common.KioskCash.DataBytes[2] := 0;
  Common.KioskCash.DataBytes[3] := BAdd;
  Common.KioskCash.DataBytes[4] := BCmd;

  Common.KioskCash.DataBytes[6] := BMin;
  Common.KioskCash.DataBytes[7] := 0;
  for I := 0 to 6 do
    Common.KioskCash.DataBytes[7] := Common.KioskCash.DataBytes[7] xor Common.KioskCash.DataBytes[I];
  SendDispenserData(Common.KioskCash.DataBytes, 8);
end;

procedure TDevice.CmdToTwoByte(BCmd, BAdd, BAdd1, BMin: Byte);
var
  I: Integer;
begin
  InitKioskCashRecord(Common.KioskCash);
  Common.KioskCash.DataBytes[0] := $FE;
  Common.KioskCash.DataBytes[1] := 8;
  Common.KioskCash.DataBytes[2] := 0;
  Common.KioskCash.DataBytes[3] := BAdd;
  Common.KioskCash.DataBytes[4] := BCmd;

  Common.KioskCash.DataBytes[6] := BMin;
  Common.KioskCash.DataBytes[7] := BAdd1;
  Common.KioskCash.DataBytes[8] := 0;
  for I := 0 to 7 do
    Common.KioskCash.DataBytes[8] := Common.KioskCash.DataBytes[8] xor Common.KioskCash.DataBytes[I];
  SendDispenserData(Common.KioskCash.DataBytes, 9);
end;

procedure TDevice.SendDispenserData(const AData: array of Byte; ALength: Integer);
var
  I: Integer;
begin
  if DispenserComPort.Active and (ALength > 0) then
  begin
    DispenserComPort.SendData(PAnsiChar(@AData[0]), ALength);
  end;
end;
