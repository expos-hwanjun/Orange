...
//     end;
     ...
     if IsDebuggerPresent or (GetOption(492) = '1') then
       AddPrintData(BuildNaverQRData('https://nid.naver.com/nidlogin.login?realname=Y&svctype=262144&url=https://m.place.naver.com/my/checkin'));
        ...
        begin
          ...
          case StoI(GetOption(103)) of
            ...
            begin
              ...
              if LengthB(vMenu)+LengthB(vMemo) <= vMenuLen then
                vTemp := RPadB(vMenu+vMemo,vMenuLen,' ')
              else
                vTemp := RPadB(Scopy(vMenu+vMemo,1,vMenuLen),vMenuLen,' ');

              vTemp := vTemp + LPadB('-'+AQty,  vQtyLen,' ')+#13;

              //메뉴명이 길때 두번째줄 출력
              if (LengthB(vMenu)+LengthB(vMemo) > vMenuLen) then
                vTemp := vTemp + Scopy(vMenu+vMemo,vMenuLen+1,LengthB(vMenu+vMemo)) +#13;
            end;
            2 : //가로세로확대
            begin
              if aGrid.Cells[GDM_DS_MENU, aRow] = 'G' then
...
begin
  ...
  begin
    if TBaeminOrderMenu(aOrderMenu.Items[vIndex]^).Name = '' then Continue;
...
//  Data: TBytes;
//begin
//  Data := BuildNaverQRData;
//  Memo1.Lines.Text := BytesToHex(Data);
