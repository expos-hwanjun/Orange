...
type
  TKioskPhoneKeyPadBF_F = class(TForm)
    KeyInLabel: TcxLabel;
...
var
  KioskPhoneKeyPadBF_F: TKioskPhoneKeyPadBF_F;
...
procedure TKioskPhoneKeyPadBF_F.FormCreate(Sender: TObject);
var vIndex :Integer;
begin
  Common.LogoCreate(Self,0);
  Self.Position := poDesigned;
  Self.ClientWidth    := 860;
...
procedure TKioskPhoneKeyPadBF_F.FormShow(Sender: TObject);
begin
  BlockInput(false);
...
procedure TKioskPhoneKeyPadBF_F.Num1ButtonClick(Sender: TObject);
begin
...
procedure TKioskPhoneKeyPadBF_F.NumBSButtonClick(Sender: TObject);
begin
  Common.KioskTouchBeep('kioskwave12');
...
function  TKioskPhoneKeyPadBF_F.GetPhoneFormat(aStr :String):String;        //전화번호에 하이픈넣기
var vTemp :String;
begin
  vTemp := aStr;
...
function TKioskPhoneKeyPadBF_F.GetPassword(aStr: String): String;
var vIndex :Integer;
begin
  Result := EmptyStr;
...
procedure TKioskPhoneKeyPadBF_F.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TKioskPhoneKeyPadBF_F.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    OKButtonClick(nil);
...
procedure TKioskPhoneKeyPadBF_F.OKButtonClick(Sender: TObject);
begin
...
procedure TKioskPhoneKeyPadBF_F.AgreeButtonClick(Sender: TObject);
begin
...
procedure TKioskPhoneKeyPadBF_F.BSButtonClick(Sender: TObject);
begin
  Common.KioskTouchBeep('kioskwave12');
...
procedure TKioskPhoneKeyPadBF_F.CancelButtonClick(Sender: TObject);
begin
  Common.KioskTouchBeep('kioskwave12');
  ModalResult := mrNo;
end;

procedure TKioskPhoneKeyPadBF_F.ClearButtonClick(Sender: TObject);
begin
  Common.KioskTouchBeep('kioskwave12');
  KeyInLabel.Hint    := '010';
  KeyInLabel.Caption := '010';
...
procedure TKioskPhoneKeyPadBF_F.CloseButtonClick(Sender: TObject);
begin
...
procedure TKioskPhoneKeyPadBF_F.CloseTimerTimer(Sender: TObject);
begin
...
procedure TKioskPhoneKeyPadBF_F.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  CloseTimer.Enabled := false;
end;

procedure TKioskPhoneKeyPadBF_F.ScannerReadEvent(const S: String);
begin
  KeyInLabel.Hint := S;
  KeyInLabel.Caption := KeyInLabel.Hint;
end;

procedure TKioskPhoneKeyPadBF_F.ShowTimerTimer(Sender: TObject);
begin
